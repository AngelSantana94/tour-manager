...
} from "../Services/SupabaseOTA.adapter";
...
const PLATFORM_LOGO: Record<string, string> = {
  guruwalk: guruwalkLogo,
  freetour: freetourLogo,
  turixe: turixeLogo,
...
function GuideAvatar({
  ...
  return (
    <div
      className={`w-${size} h-${size} rounded-full bg-primary/15 border border-primary/20 overflow-hidden flex items-center justify-center text-primary text-xs font-bold shrink-0`}
    >
      {guide.avatarUrl ? (
        <img
          src={guide.avatarUrl}
...
function GuideAssignPanel({
  ...
  const allAssignedIds = allEvents.flatMap((e) =>
    ((e.meta?.tourGuides as GuideProfile[]) ?? []).map((g) => g.id),
  );
  const uniqueAssignedIds = [...new Set(allAssignedIds)];

  const paxByGuide = (guideId: string) => {
    const allRes = allEvents.flatMap(
  ...
  const handleDistribute = async () => {
    ...
    const allRes = allEvents
      ...
      .filter(
        (r) => r.status !== "cancelled" && r.platform !== "tuguiaenbrujas",
      )
      .sort(
        (a: any, b: any) => b.adults + b.children - (a.adults + a.children),
      );
    ...
    if (assignedGuides.length === 1) {
      ...
      assignedGuides.forEach((g) => {
        paxCount[g.id] = 0;
      });

      for (const res of allRes) {
        const guideId = assignedGuides.reduce(
          (min, g) => (paxCount[g.id] < paxCount[min] ? g.id : min),
          assignedGuides[0].id,
        );
  ...
  const availableGuides = allGuides.filter(
    (g) => !uniqueAssignedIds.includes(g.id),
  );
...
function AttendanceSelector({
  ...
  const handleSubmit = async () => {
    ...
    const attendedCount =
      state === "all" ? totalPax : state === "partial" ? partialCount : 0;
    await supabase
      .from("reservations")
    ...
    <div className="mt-3 pt-3 border-t border-base-content/5">
      <div className="flex items-center gap-3 flex-wrap">
        <label className="flex items-center gap-1.5 cursor-pointer">
          <input
            type="radio"
            name={`att-${reservation.id}`}
            checked={state === "all"}
            onChange={() => {
              setState("all");
              setSaved(false);
            }}
            className="radio radio-xs border-base-content/30"
          />
      ...
      {state === "partial" && (
        <div className="flex items-center gap-2 mt-2">
          <button
            onClick={() => setPartialCount((v) => Math.max(0, v - 1))}
            className="btn btn-xs btn-outline btn-square border-base-content/20"
          >
            −
          </button>
          ...
          <button
            onClick={() => setPartialCount((v) => Math.min(totalPax, v + 1))}
            className="btn btn-xs btn-outline btn-square border-base-content/20"
          >
            +
          </button>
        ...
        <div className="mt-2 pt-2 border-t border-base-content/5 flex items-center gap-2">
          ...
          {isAdmin ? (
            <select
              value={guideId ?? ""}
              onChange={(e) =>
                onGuideChange(reservation.id, e.target.value || null)
              }
            ...
            <span className="text-xs opacity-50">
              {assignedGuide?.name ?? "Sin asignar"}
            </span>
          )}
        </div>
      )}
...
function ReservationList({
  ...
  const otaReservations = reservations.filter(
    (r) => (r as any).platform !== "tuguiaenbrujas",
  );
  const active = otaReservations.filter((r) => r.status !== "cancelled");
  const cancelled = otaReservations.filter((r) => r.status === "cancelled");
    ...
    if (tourGuides.length === 0) {
      return active.map((r) => (
        <ReservationCard
          ...
          onAttendanceConfirm={onAttendanceConfirm}
        />
      ));
    }
      ...
      <div>
        ...
        {active.length === 0 ? (
          <div className="flex flex-col items-center py-10 opacity-25 gap-2">
            ...
            <span className="text-sm">Sin reservas de OTAs</span>
          </div>
        ) : (
...
function EventInfo({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="border border-base-content/10 rounded-2xl p-4">
        <h3 className="text-sm font-bold mb-3">Resumen OTAs</h3>
        <div className="flex flex-col gap-2">
        ...
        {(() => {
          const guides = allEvents.flatMap(
            (e) => (e.meta?.tourGuides as GuideProfile[]) ?? [],
          );
          ...
          return (
            <>
              ...
              <div className="flex flex-col gap-2">
                {unique.map((guide) => {
                  const guideRes = otaReservations.filter(
                    (r) =>
                      (r as any).guideId === guide.id &&
                      r.status !== "cancelled",
                  );
                  const confirmedCount = guideRes.filter(
                    (r) => r.attended,
                  ).length;
                  const confirmedPax = guideRes.reduce(
                    (a, r) => a + ((r as any).attended_count ?? 0),
                    0,
                  );
                  ...
                  return (
                    <div key={guide.id} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] opacity-40">
                          {confirmedCount}/{total} reservas · {confirmedPax} pax
                          confirmados
                        </p>
                      </div>
                    </div>
...
export default function OtaEventBody({
  event,
  allEvents,
  reservations,
  mobileView,
